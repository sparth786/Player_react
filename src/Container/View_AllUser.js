...
class View_AllUser extends Component {
    ...
    Displaydata = () => {
        ...
        return data.map((value) => {
            ...
            return (
                <tr><td><input type="checkbox" onChange={this.onchecked} name="checkdata" value={_id} checked={this.state.checked_userid.includes(_id)} /></td><td>{username}</td><td>{moment(dob).format('DD-MM-YYYY')}</td><td>{role}</td><td>{batsmanstyle}</td><td>{bowlerstyle}</td><td>{team}</td><td><img src={baseURL + profile} onClick={this.zoomimage} width="80px" height="80px" class="rounded" /></td><td><Button outline color="success" onClick={this.btn_edit} id={_id}><FaEdit /></Button></td><td><Button outline color="danger" onClick={(e) => window.confirm("Are you sure you wish to delete this item?") &&
